...
while(play == 'sim'){
    ...
    for(let i = 0; i < rodadas; i++){
        playerChoice = prompt("Escolha 'Pedra', 'Papel ou 'Tesoura': ").toLowerCase();
        computerChoice = choices[Math.floor(Math.random()*3)];
        
        while (playerChoice != choices[0] && playerChoice != choices[1] && playerChoice != choices[2]) 
        playerChoice = prompt("Escolha Inválida! Escolha 'Pedra', 'Papel ou 'Tesoura': ").toLowerCase();
